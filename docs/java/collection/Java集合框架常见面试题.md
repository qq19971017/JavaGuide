...
**面试问题**
1. ArrayList和LinkedList的区别？ 从底层结构，扩容机制，适用场景。
2. ArrayList和LinkedList的适用场景？ ArrayList更适合需要快速随机访问的场景，不是和存储大量数据(因为底层是数组需要分配连续内存)，LinkedList适合增删频繁的场景。
3. 有哪些 ArrayList和LinkedList结合的的数据结构？ hashmap
4. 
