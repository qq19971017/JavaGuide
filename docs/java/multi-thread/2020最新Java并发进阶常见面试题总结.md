...
2. 既然volatile关键字可以解决可见性，为什么不所有变量都使用。
3. volatile和syn关键字区别。
4. 问volatile不能解决原子性，那怎么解决的？ 原子类
5. synchronized关键字的底层实现
6. synchronized的原理、synchronized是否可重入、synchronized如何实现可重入、synchronized锁自旋会先来先到吗？
7. synchronized的锁升级过程？
8. synchronized和ReentrantLock、实现原理以及区别
