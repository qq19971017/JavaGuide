...
**面试问题**
1. volatile关键字。 从多线程缓存一致性，内存屏障，happens-before角度来说。保证共享变量的可见性。
2. volatile如何保证可见性和有序性？ 
可见性：缓存一致性，写入主存的时候通过总线嗅探机制告知其他线程该变量无效，等到其他线程修改该变量，会重新从主存中读取。
有序性：使用内存屏障指令，再volatile写操作前后分别添加，storestore， storeload， 在volatile读操作后面添加loadload，loadstore指令。
4. 既然volatile关键字可以解决可见性，为什么不所有变量都使用。
5. volatile和syn关键字区别。
6. 问volatile不能解决原子性，那怎么解决的？ 原子类：底层是使用了cas和volatile
7. synchronized关键字的底层实现
8. synchronized的原理、synchronized是否可重入、synchronized如何实现可重入、synchronized锁自旋会先来先到吗？
9. synchronized的锁升级过程？
10. synchronized和ReentrantLock、实现原理以及区别
